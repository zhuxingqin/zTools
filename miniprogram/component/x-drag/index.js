...
Component({
	...
	methods: {
  	...
  	goWorkArae(e){
    	...
    	wx.navigateTo({
      	url: '/pages/home/workArea/workArea?wid=' + currentItem.data._id + '&wname=' + currentItem.data.name,
    	})
  	},
		scroll(e) {
		...
		longPress(e) {
			...
			let tranX = startPageX - itemDom.width / 2,
				...
				tranY = tranY>0?tranY:itemWrapDom.top;
		...
		touchMove(e) {
			...
			let tranX = currentClientX - itemDom.width / 2 , 
					tranY = currentClientY - itemDom.height / 2 - itemWrapDom.top;
			// 设置当前激活元素偏移量
		...
		touchStart(e){
			let startTouch = e.changedTouches[0];
			if (!startTouch) return;
			this.positionView(startTouch,'start');
			
		},
		touchEnd(e) {
			let startTouch = e.changedTouches[0];
			this.positionView(startTouch,'end')
			if (!this.data.dragging) return;
			this.setData({
				transition:true
			},()=>{
		 ...
		 * 归位
		 */
		positionView(startTouch,type){
			if(type == 'end'){
				let item = this.data.list.find(item=>item.id==this.data.currentItem.id);
				var tranX = (item.tranX * 135) - this.data.scrollLeft;
			}else{
				this.startKey = this.getKeyByClientX(startTouch.clientX);
				var currentItem = this.data.list.find(item => item.sortKey === this.startKey);
				var tranX = currentItem.tranX * 135 - this.data.scrollLeft;
			}
			this.setData({
				transition:true,					
				tranX,
		...
		triggerCustomEvent(list, type) {
			...
			let index = app.workAreaList.findIndex(item=>item._id==currentItem.id) 
			if(type == 'sortend'){
				if(index == currentItem.sortKey){
					clearInterval(this.interval)
					this.interval = null;
					return
				}
		...
		initDom(isMaxScroll) {
			...
			this.createSelectorQuery().select(".item").boundingClientRect((res) => {
				...
				let wrapStyle = `width: ${this.data.list.length * res.width}px;`
				this.setData({
					wrapStyle
				});
